...
def execute_test_case_by_hybrid(test_step_sheet_name,test_datas_dict):
...
def execute_test_case_by_keyword(test_step_sheet_name):
    wb = ExcelUtil(test_data_file_path)
    print("要执行的sheet name:", test_step_sheet_name)
    if not test_step_sheet_name in wb.get_sheet_names():  # 判断了一下测试步骤sheet名字在不在
        print("设定的测试用例sheet名称不存在！", test_step_sheet_name)
    ...
    try:
        ...
        if flag:
            ...
            result =  "失败"
        test_result = {"测试时间":test_time,"测试结果":result}
    except Exception  as e:
        traceback.print_exc()
...
def execute_testcase_by_excel(excel_file_path):
    test_cases = get_test_cases(excel_file_path, test_case_info_sheet)
    for test_case in test_cases:
        print(test_case)
        # if 的部分通过判断是否有测试数据sheet名称
        if test_case["测试数据sheet名称"] and re.search(r"\S+", test_case["测试数据sheet名称"]):
            test_step_sheet_name = test_case["测试步骤sheet名称"]
            test_data_sheet_name = test_case["测试数据sheet名称"]
            test_datas_dict = get_test_data_from_sheet(excel_file_path, test_data_sheet_name)
            a_group_test_data_result = execute_test_case_by_hybrid(test_step_sheet_name, test_datas_dict)
            ...
            test_step_sheet_name = test_case["测试步骤sheet名称"]
            test_case_result = execute_test_case_by_keyword(test_step_sheet_name)
            wb = ExcelUtil(excel_file_path)
            wb.set_sheet_by_name(test_result_sheet)
            test_case["测试时间"] = test_case_result["测试时间"]
            test_case["测试结果"] = test_case_result["测试结果"]
            test_case_head = list(test_case.keys())
            wb.write_a_line_in_sheet(test_case_head)
            test_case = list(test_case.values())
            wb.write_a_line_in_sheet(test_case)
            wb.save()


if __name__ == "__main__":
    execute_testcase_by_excel(test_data_file_path)
